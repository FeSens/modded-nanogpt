...
class GlobalaBReLU(nn.Module):
    def __init__(self, alpha: nn.Parameter, beta: nn.Parameter):
        super().__init__()
        self.alpha = alpha
        self.beta = beta
...
class MLP(nn.Module):
    def __init__(self, dim: int, alpha: nn.Parameter, beta: nn.Parameter):
        super().__init__()
        hdim = 4 * dim
        self.c_fc = CastedLinear(dim, hdim)
        self.c_proj = CastedLinear(hdim, dim)
        self.c_proj.weight.detach().zero_() # zero init suggested by @Grad62304977
        self.aBReLU = GlobalaBReLU(alpha=alpha, beta=beta)
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, layer_idx: int, alpha: nn.Parameter, beta: nn.Parameter):
        super().__init__()
        # skip attention of blocks.7 (the 8th layer) by @YouJiacheng
        self.attn = CausalSelfAttention(dim, num_heads, max_seq_len) if layer_idx != 7 else None
        self.mlp = MLP(dim, alpha=alpha, beta=beta)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.blocks = nn.ModuleList([Block(model_dim, num_heads, max_seq_len, i, alpha=self.alpha, beta=self.beta) for i in range(num_layers)])
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        # loss = self.weighted_cross_entropy(logits, target_seq, normalize="loss")
    ...
    def weighted_cross_entropy(self, logits: Tensor, target_seq: Tensor, alpha: float = 0.2, normalize="area"):
        ...
        weights = pos_weights.repeat(batch_size)

        if normalize == "loss":
            l_sum = unreduced_loss.sum()
            wl_sum = (weights * unreduced_loss).sum()
            weights = weights * (l_sum / wl_sum)
        ...
        return weighted_loss.sum() if self.training else unreduced_loss.mean()
