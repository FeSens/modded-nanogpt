...
class GlobalaReLU(nn.Module):
    def __init__(self, alpha: nn.Parameter):
        super().__init__()
        self.alpha = alpha
...
class MLP(nn.Module):
    def __init__(self, dim: int, alpha: nn.Parameter):
        super().__init__()
        hdim = 4 * dim
        self.c_fc = CastedLinear(dim, hdim)
        self.c_proj = CastedLinear(hdim, dim)
        self.c_proj.weight.detach().zero_() # zero init suggested by @Grad62304977
        self.aReLU = GlobalaReLU(alpha=alpha)
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, layer_idx: int, alpha: nn.Parameter):
        super().__init__()
        # skip attention of blocks.7 (the 8th layer) by @YouJiacheng
        self.attn = CausalSelfAttention(dim, num_heads, max_seq_len) if layer_idx != 7 else None
        self.mlp = MLP(dim, alpha=alpha)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.blocks = nn.ModuleList([Block(model_dim, num_heads, max_seq_len, i, alpha=self.alpha) for i in range(num_layers)])
